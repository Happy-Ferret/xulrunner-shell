...
http://developer.mozilla.org/en/docs/nsIDirectoryServiceProvider2
http://developer.mozilla.org/en/docs/nsIDirectoryServiceProvider
http://developer.mozilla.org/en/docs/nsIDirectoryService
...
File.prototype.append = function( path, _native ){
  var parts = path.split(/\\\//); // either \ or / separators
  parts.foreach(function( part ){
    if(_native) this.nsIFile.appendNative( part );
...
File.prototype.copyTo = function( name, dir, dereference, _native ){
  if(this.isSymlink()&&dereference)
    return this.copyToFollowingLinks( name, dir, _native );
  if(_native) this.nsIFile.copyToNative( dir, name );
...
File.prototype.copyToFollowingLinks = function( name, dir, _native ){
  if(_native) this.nsIFile.copyToNativeFollowingLinks( dir, name );
...
File.prototype.moveTo = function( name, dir, _native ){
  if(_native) this.nsIFile.moveToNative( dir, name );
  else this.nsIFile.moveTo( dir, name );
  return this;
};

// renames a file
File.prototype.rename = function( name, _native ){
  return this.moveTo( name, null, _native );
};
...
// os specific path
File.prototype.path = function( _native ){
  return _native ? this.nsIFile._nativePath : this.nsIFile.path ;
};

// dereferences links
File.prototype.target = function( _native ){
  return _native ? this.nsIFile._nativeTarget : this.nsIFile.target ;
};
...
// name of item
File.prototype.leafName = function( _native ){
  if(_native) return this.nsIFile.leafNameNative;
